...
#Note : This script has POD documentation.You can use POD tools to view it,    \
    like:
...
if (defined $output_file) {
  open($ofh, '>', $output_file) or die
      "Cannot write to file '$output_file': $!";
} else {
  $ofh = *STDOUT;
}
...
#define $ {heap_name} _FREE free
  ...
  *_${heap_name} _HEAD = NULL;
  ...
  void *${heap_name} _alloc(const char *trace, size_t size) {
...
#if ALLOCPP_DEBUG
      fprintf("Refget error: `%s` is larger than maximum trace size (%d)",
              trace, TRACE_SIZE);
#endif
      return NULL;
    }
    ...
    struct __$ {
      heap_name
    } __ *node = ${heap_name} _MALLOC(sizeof(struct __${heap_name} __));

    if (node == NULL) {
#ifdef ALLOCPP_DEBUG
      fprintf("Allocation error: Failed to allocate space for `%s` trace",
              trace);
#endif
    }
  ...
  void ${heap_name} _free(void *mem) {
    struct __$ {
      heap_name
    } __ **indirect = &_${heap_name} _HEAD;
    while (*indirect != NULL) {
      if ((*indirect)->mem == mem) {
        struct __$ {
          heap_name
        } __ *obsolete = *indirect;
...
foreach
  my $heap_name(@heap_annotations) { emit($heap_name); }

#Print the normal lines or other preprocessing as needed
foreach
  my $line(@normal_lines) { print $ofh $line; }
